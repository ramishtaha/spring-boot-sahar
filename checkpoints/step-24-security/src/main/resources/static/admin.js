...
// Step 24: writes require the admin. We keep the Basic-auth credentials in sessionStorage (cleared when the
// tab closes) and attach them only to writes - reads are public. On the first write we prompt once.
function authHeader(method) {
    if (method === 'GET') return null; // reads are public; never send credentials for them
...
async function api(method, path, body) {
    ...
    const auth = authHeader(method);
    if (auth) opts.headers['Authorization'] = auth;
    const resp = await fetch(path, opts);
    if (resp.status === 401) {
        sessionStorage.removeItem('sahar-auth'); // wrong/expired credentials - prompt again next write
        toast('Sign in failed — check the admin password and try again.', 'err');
    }
    let parsed = null;
