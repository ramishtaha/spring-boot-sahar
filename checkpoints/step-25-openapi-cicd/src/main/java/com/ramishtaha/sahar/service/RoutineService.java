...
import com.ramishtaha.sahar.event.LocationChangedEvent;
...
public class RoutineService {
    ...
    public RoutineService(MetaRepository meta, PrayerTimesRepository prayerTimes, BlockRepository blocks,
                          ...
                          MealPlanRepository mealPlan, ApplicationEventPublisher events) {
        this.meta = meta;
    ...
    /**
     * Save the user-chosen location (from geolocation, search, or the map) and announce the change.
     *
     * <p>{@code @Transactional} so the save and the event publication share one transaction: the
     * {@code LocationChangedEvent} is only delivered to {@code AFTER_COMMIT} listeners once the row is durably
     * committed (see {@link com.ramishtaha.sahar.event.PrayerTimesRecalculator}). Notice this method does NOT
     * recompute prayer times itself - it just states the fact that the location changed and lets a listener
     * react. That is the decoupling the event buys us.
     */
    @Transactional
