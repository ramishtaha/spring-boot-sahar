...
class RoutineApiIntegrationTest {
    ...
    void changingLocationRecalculatesPrayerTimes() throws Exception {
        ...
        try {
            ...
            mvc.perform(put("/api/location").contentType("application/json").content(seedLocation));
            mvc.perform(put("/api/prayer-times").contentType("application/json").content(seedPrayerTimes));
        }
    }
