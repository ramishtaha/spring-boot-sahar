...
> [!NOTE]
> **Version context.** Persistence defaults drift across releases — starter names, the H2 console module, connection-pool wiring, and JPA's behavior all shifted between Spring Boot 3.x and 4. Older tutorials also assume Java 17 and the `javax.persistence`/`javax.sql` packages, which became `jakarta.*` in the Boot 3 / Spring 6 era. This page targets **Spring Boot 4.0.6 / Java 25 / PostgreSQL 17**. For the consolidated "what moved and when" story, see [Version deltas](../../reference/cheatsheet-version-deltas.md).
...
## ⚖️ SQL vs. ORM, and the N+1 query trap
...
These come up constantly in backend interviews. Each answer is drawn straight from this page.

**Q: What's the difference between an embedded database and a server database?**
An **embedded DB** (H2, SQLite) runs *inside* your application's process — it's a library on the classpath, dies with the JVM, and needs no separate server to install or operate. A **server DB** (PostgreSQL, MySQL) is a *separate process* that outlives every app restart, serves many concurrent clients, and gives you row-level locking and MVCC (Multi-Version Concurrency Control — readers and writers don't block each other). You reach for embedded in dev/test/demos and a server DB in production.

**Q: Why is "just store it in a JSON file" usually the wrong call?**
To do flat files *correctly* you'd hand-write atomic writes (temp file + fsync + rename), file locking so concurrent requests don't clobber each other, and your own query/filter/update logic. An embedded database gives you durability, atomicity, concurrency, and SQL for *less* code — the hard parts are already written. The file looks simpler but secretly asks you to reimplement a database.

**Q: What is the N+1 query problem, and how do you fix it?**
You load N parent rows in **1** query, then the ORM lazily loads each parent's children in **N** more queries — `N + 1` round trips where 1 or 2 would do. Spot it by enabling SQL logging and watching identical `SELECT`s repeat, or by latency that scales with row count. Fix it in ORM land with a **fetch join** (`JOIN FETCH`), an `@EntityGraph`, or batch fetching (`@BatchSize`) — every fix is really you reaching back toward the SQL you'd have written by hand.

**Q: Why choose `JdbcTemplate` over raw JDBC, and over JPA?**
Raw JDBC forces a dangerous connection/statement/`ResultSet` resource dance where a mistake leaks connections until the pool starves. `JdbcTemplate` deletes that plumbing — it borrows a pooled connection, runs the statement, maps rows via a `RowMapper`, and cleans up — while you still own every query (parameterized, so injection-safe). JPA goes further by *generating* the SQL, which trades control for less code and adds "magic" (a persistence context, dirty-checking, lazy loading) you must eventually understand.

**Q: How does swapping H2 for PostgreSQL cost zero Java?**
Because the two axes — *where data lives* and *how code talks to it* — are independent. Sahar commits to standard SQL through `JdbcTemplate`, puts Postgres behind a Spring **profile**, and only the `spring.datasource.*` connection properties change. The portable schema (`GENERATED BY DEFAULT AS IDENTITY`, `BOOLEAN`) and the repository code run unchanged on both engines.

See the central [interview prep bank](../../reference/interview-prep.md) for more, including the transaction and ACID questions in [Transactions & ACID](./transactions-and-acid.md).
...
- Theory: [Transactions & ACID](./transactions-and-acid.md) — why `@Transactional` makes the multi-statement `replace(...)` atomic.
...
- Reference: [Version deltas](../../reference/cheatsheet-version-deltas.md) — the Boot 3.x → 4 persistence story in one place.
- Reference: [Interview prep](../../reference/interview-prep.md) — the central interview question bank.
